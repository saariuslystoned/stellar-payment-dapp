...
export function TokenDashboard({ publicKey, networkPassphrase }: TokenDashboardProps) {
    const [balances, setBalances] = useState<{ smoky: string; zmoke: string }>({ smoky: '0', zmoke: '0' });
    ...
    return (
        <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                <h3 className="text-slate-400 text-sm font-medium mb-1">SMOKY Holdings</h3>
                <div className="text-2xl font-bold">{balances.smoky}</div>
                <div className="text-emerald-500 text-xs mt-1">Governance & Asset</div>
            </div>
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                <h3 className="text-slate-400 text-sm font-medium mb-1">ZMOKE Rewards</h3>
